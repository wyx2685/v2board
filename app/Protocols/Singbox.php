...
class Singbox
{
    ...
    protected function buildHysteria($password, $server, $user)
    {
        ...
        if (is_null($server['version']) || $server['version'] == 1) {
            ...
            $array['tag'] = $server['name'];
            $array['type'] = 'hysteria';
            $array['up_mbps'] = $user->speed_limit ? min($server['down_mbps'], $user->speed_limit) : $server['up_mbps'];
            $array['down_mbps'] = $user->speed_limit ? min($server['up_mbps'], $user->speed_limit) : $server['down_mbps'];
