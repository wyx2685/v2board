...
class ClashMeta
{
    ...
    private function buildProxy($server)
    {
        ...
        if (method_exists($this, $method)) {
            return $this->$method($this->user['uuid'], $server);
        }
        return [];
    }
    ...
    private function buildTrojan($password, $server)
    {
        ...
        if (!empty($server['allow_insecure'])) $array['skip-cert-verify'] = $server['allow_insecure'];
    ...
    private function buildCommonSettings($id, $server)
    {
        return [
            ...
            'udp' => true,
            'skip-cert-verify' => $server['insecure'] ?? false,
    ...
    {
        return [
            'skip-cert-verify' => $tlsSettings['allowInsecure'] ?? false,
    ...
    private function buildVlessTlsSettings($server)
    {
        ...
        $settings = [
            'skip-cert-verify' => $tlsSettings['allow_insecure'] ?? false,
            'flow' => $server['flow'] ?? '',
    ...
    private function buildNetworkSettings($server)
    {
        ...
        if ($network === 'tcp' || $network === 'ws' || $network === 'grpc' || $network === 'h2') {
            ...
            $opts = "{$network}-opts";
            $settings[$opts] = $this->buildSpecificNetworkSettings($network, $server['networkSettings']);
        }
        return $settings;
    }
